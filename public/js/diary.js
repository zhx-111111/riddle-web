...
function markWriting(on) {
  ...
  if (on) {
    ...
    // Arm the fade button so user can manually drop UI to 15%
    setFadeActive(true);
  } else {
    if (state.replyActive) return;
    state.writingGrace = setTimeout(() => {
      document.body.classList.remove("writing");
      // Disarm fade button when fully idle
      setFadeActive(false);
    }, 900);
  }
}
  ...
  try {
    ...
    for await (const ev of sseEvents(resp)) {
      if (ev.type === "ink") {
        ...
        document.body.classList.add("writing");
        // Arm fade button while AI is writing
        setFadeActive(true);
        hand.write(ev.text);
    ...
    // Auto-fade after fadeDelayMs (default 30s)
    const fadeMs = (state.config?.fadeDelayMs) || 30000;
    state.autoFadeTimer = setTimeout(() => {
      if (state.pageHasReply && !state.busy && !state.conjuring) {
        hand.fadeOutClear(420);
        state.pageHasReply = false;
        // Disarm fade button after auto-fade
        setFadeActive(false);
      }
    }, fadeMs);
  } catch (e) {
...
function newPageIfNeeded() {
  ...
  if (state.busy) {
    ...
    document.body.classList.remove("writing");
    btnReset.classList.add("hidden");
    btnFade.classList.add("hidden");
    whisper("");
  }
  if (state.autoFadeTimer) { clearTimeout(state.autoFadeTimer); state.autoFadeTimer = null; }
  return false;
}
...
function toggleMusic() {
  const url = state.config?.musicUrl || bgMusic.dataset.url || "";
  if (!url && !bgMusic.src) return;
  if (bgMusic.src !== url && url) bgMusic.src = url;
...
// Long-press music button (600ms) → open file picker to upload music
let pressTimer = null;
...
btnMusic.addEventListener("pointerup", () => {
...
musicFileInput.addEventListener("change", (e) => {
  ...
  try { localStorage.setItem("trd.musicName", file.name); } catch {}
});

// ----------------------------------------------------------------- fade button
// Only active during user writing or AI writing.
// User can click to manually force 15% UI transparency.
// Automatically reverts when neither user nor AI is writing.

let userFaded = false;
let fadeArmed = false; // true when the button is available for clicking

function setFadeActive(active) {
  fadeArmed = active;
  btnFade.disabled = !active;
  if (active) {
    btnFade.classList.remove("hidden");
  } else {
    // Deactivate: clear any user-forced fade
    userFaded = false;
    document.body.classList.remove("writing");
...
btnFade.addEventListener("click", () => {
  ...
  if (userFaded) {
    ...
    // Only remove writing class if AI is also not active
    if (!state.replyActive) document.body.classList.remove("writing");
    btnFade.classList.remove("active");
  }
});
...
function handleIconClick() {
  if (!state.config?.enableEasterEgg) return; // disabled via admin
  state.iconClicks++;
