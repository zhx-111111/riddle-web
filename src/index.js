...
export default {
  async fetch(req, env) {
    ...
    if (p.startsWith("/api/")) {
      ...
      if (p === "/api/admin/upload-music" && req.method === "POST") return adminUploadMusic(req, env);

      // WeChat verification file: serve from KV if configured
      if (env.DIARY_KV) {
        const cfg = await loadConfig(env);
    ...
    // Serve uploaded music files from KV (path: /music/<filename>)
    if (p.startsWith("/music/") && env.DIARY_KV) {
      const fname = decodeURIComponent(p.slice(7));
      const data = await env.DIARY_KV.get("music:" + fname, { type: "arrayBuffer" });
      if (data) {
        const ct = fname.endsWith(".mp3") ? "audio/mpeg" :
                  fname.endsWith(".wav") ? "audio/wav" :
                  fname.endsWith(".ogg") ? "audio/ogg" :
                  fname.endsWith(".m4a") ? "audio/mp4" : "application/octet-stream";
...
async function adminUploadMusic(req, env) {
  if (!(await checkToken(env, req))) return json({ error: "unauthorized" }, 401);
  if (!env.DIARY_KV) return json({ error: "未绑定 KV" }, 409);
  let payload;
  try { payload = await req.json(); } catch { return json({ error: "bad json" }, 400); }
  const name = String(payload.name || "").replace(/[^a-zA-Z0-9._-]/g, "").slice(0, 120);
  const b64 = String(payload.data || "");
  const type = String(payload.type || "audio/mpeg");
  if (!name || !b64) return json({ error: "missing name or data" }, 400);
  try {
    const bin = Uint8Array.from(atob(b64), (c) => c.charCodeAt(0));
    await env.DIARY_KV.put("music:" + name, bin, { metadata: { type } });
    // Auto-fill the music URL in config
    const cfg = await loadConfig(env);
    const musicUrl = new URL("/music/" + encodeURIComponent(name), req.url).toString();
    const merged = mergeConfig({ ...cfg, musicUrl });
    await env.DIARY_KV.put("config", JSON.stringify(merged));
    return json({ ok: true, url: musicUrl, config: merged });
  } catch (e) {
    return json({ error: "upload failed: " + String(e.message || e).slice(0, 200) }, 500);
  }
}
